...
🔤 **Список терминов по умолчанию** (27 штук) построен замерами по реестру и покрывает четыре группы: wildcard-словоформы (`базов*`, `радиотелефон*`, `сотов*`, `GSM*`, `LTE*` + опечатки), аббревиатуры (`ПРТО`, `РЭС`, `BTS`), операторов (`МТС`, `МегаФон`, `Вымпел*`, `Ростелеком*`, …) и варианты с перепутанным кириллица/латиница алфавитом (`МТC`, `MTC`, `BТS`, `Cкартел*`, …) — такие написания реально встречаются в реестре и находятся только так. Даёт +13,6 % документов к поиску только по «базов»-формам; лишнее отсеивает ML-классификатор.
