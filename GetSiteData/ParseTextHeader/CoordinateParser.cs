...
public static partial class CoordinateParser
{
    ...
    [GeneratedRegex(@"(?:[Шш]ирота[:\s]*|[Лл]атитуд[:\s]*)(\d{2,3},\d{4,})°[^,]*(?:[Дд]олгота[:\s]*|[Лл]онгитуд[:\s]*)(\d{2,3},\d{4,})°", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"\(\s*(\d{2,3}\.[\s\d]{3,})\s*[,;]\s*(\d{2,3}\.[\s\d]{3,})\s*\)", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"(\d{2,3}\.\s*\d{4,})[,/\s]+(\d{2,3}\.\s*\d{4,})", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"(?:С\.Ш\.|с\.ш\.)\s*(\d{1,3})°\s*(\d{1,2})\s*'\s*\.?(\d+(?:[.,]\d+)?)\s*""?", RegexOptions.IgnoreCase, 2000)]
    private static partial Regex LatDmsRuRx();

    // «(42°3'50.8") N» — между кавычкой секунд и буквой направления бывает скобка (05-01-02)
    [GeneratedRegex(@"(\d{1,3})°\s*(\d{1,2})\s*'\s*\.?(\d+(?:[.,]\d+)?)""[)\s]*[Nn]", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"\bN\s*(\d{1,3})°\s*(\d{1,2})\s*'\s*\.?(\d+(?:\s*[.,]\s*\d+)?)\s*[""°]?", RegexOptions.None, 2000)]
    private static partial Regex LatDmsEnPrefixRx();

    // Долгота в DMS: «В.Д. 94°22'43.6"» / «94°22'43.6"E»
    [GeneratedRegex(@"(?:В\.Д\.|в\.д\.)\s*(\d{2,3})°\s*(\d{1,2})\s*'\s*\.?(\d+(?:[.,]\d+)?)\s*""?", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"(\d{2,3})°\s*(\d{1,2})\s*'\s*\.?(\d+(?:[.,]\d+)?)""[)\s]*[EeЕе]", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"[EЕ]\s*(\d{2,3})°\s*(\d{1,2})\s*'\s*\.?(\d+(?:[.,]\d+)?)\s*[""°]?", RegexOptions.None, 2000)]
    private static partial Regex LonDmsEnPrefixRx();

    // «(N 52.03975° E113.474028°)» — то же, но в десятичных градусах
    [GeneratedRegex(@"\bN\s*(\d{2,3}[.,]\d{4,})\s*°?\s*[,;]?\s*E\s*(\d{2,3}[.,]\d{4,})", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"(\d{1,3})\s*°\s*(\d{1,2})\s*'\s*\.?(\d+(?:\s*[.,]\s*\d+)?)\s*""?[)\s]*([СсCcNnюЮSs]\.?\s*[Шш]?\.?)[,;:/\s\-–—]*(?:и\s+)?[EeЕе]?\s*(\d{2,3})\s*°\s*(\d{1,2})\s*'\s*\.?(\d+(?:\s*[.,]\s*\d+)?)\s*""?[)\s]*([ВвBbEeЕеЗзWw]\.?\s*[Дд]?\.?)", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"(\d{1,3})[°\-]\s*(\d{1,2})['\-]\s*\.?(\d+(?:[.,]\d+)?)[""'\s]*\s*(?:град\.?\s*)?(?:с\.ш|N)\.?[;\s,]*(\d{2,3})[°\-]\s*(\d{1,2})['\-]\s*\.?(\d+(?:[.,]\d+)?)[""'\s]*\s*(?:град\.?\s*)?(?:в\.д|E)", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"С\.Ш\.\s*:\s*(\d{2,3}[.,]\d{4,})\s*,\s*В\.Д\.\s*:?\s*(\d{2,3}[.,]\d{4,})", RegexOptions.IgnoreCase, 2000)]
    private static partial Regex ColonDirRx();

    // «55.882170(С.Ш.), 37.548570(В.Д.)» — цифры слитно со скобкой (77-01-09)
    [GeneratedRegex(@"(\d{2,3}[\.,]\d{4,})\s*\(С\.Ш\.\)[,;\s]+(\d{2,3}[\.,]\d{4,})\s*\(В\.Д\.\)", RegexOptions.IgnoreCase, 2000)]
    private static partial Regex ParenDirRx();

    // «46.055931СШ 40.885779ВД» — цифры слитно с направлением (23-КК-10)
    [GeneratedRegex(@"(\d{2,3}[\.,]\d{4,})\s*СШ[,;\s]+(\d{2,3}[\.,]\d{4,})\s*ВД", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"(?:WGS\s*)?(\d{2,3}[\.,]\d+)\s*(?:[°""'оО,]|град?\.?)*\s*(?:\([^)]{5,30}\))?\s*(?:[°""'оО,]|град?\.?)*\s*(с\.ш\.?|ю\.ш\.?|n|s)[,;/\s\-–—]+(?:WGS\s*)?(\d{2,3}[\.,]\d+)\s*(?:[°""'оО,]|град?\.?)*\s*(?:\([^)]{5,30}\))?\s*(?:[°""'оО,]|град?\.?)*\s*(в\.д\.?|з\.д\.?|e|w)", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"(\d{2,3})[°ю](\d{4,9})[""']{0,2}\s*[СC]\.?\s*[Шш]\.?[^\d]{0,10}(\d{2,3})[°ю.](\d{4,9})[""']{0,2}\s*[ВB]\.?\s*[Дд]\.?", RegexOptions.IgnoreCase, 2000)]
    ...
    // вост. долготы».
    [GeneratedRegex(@"(\d{1,3})\s*°\s*(\d{1,2})\s*'\s*([\d.,]+)\s*""?\s*сев[^,;\n]{0,12}[,;\s]+(\d{1,3})\s*°\s*(\d{1,2})\s*'\s*([\d.,]+)\s*""?\s*вост", RegexOptions.IgnoreCase, 2000)]
    private static partial Regex WordDmsRx();

    // «53°31.10.702. с.ш., 78°9.16.38. в.д.» — минуты и секунды через точки (22-01-03, 2023).
    [GeneratedRegex(@"(\d{1,3})°(\d{1,2})\.(\d{1,2}(?:\.\d{1,3})?)\.?\s*с\.?\s*ш\.?[^\d]{0,10}(\d{1,3})°(\d{1,2})\.(\d{1,2}(?:\.\d{1,3})?)\.?\s*в\.?\s*д\.?", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"(\d{1,3})°(\d{1,2})[?'](\d{1,2})(?:[.,][^\s?'""]{0,4})?[?'""]{0,2}\s*[СC]\.?\s*[Шш]\.?[^\d]{0,10}(\d{1,3})°(\d{1,2})[?'](\d{1,2})(?:[.,][^\s?'""]{0,4})?[?'""]{0,2}\s*[ВB]\.?\s*[Дд]\.?", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"(?:с\.ш\.|СШ|Ш|широта)\s*[:=]?\s*(\d{1,3})\s*°\s*(\d{1,2})\s*'\s*(\d+(?:[.,]\d+)?)\s*""?[,;\s]*(?:в\.д\.|ВД|Д|долгота)\s*[:=]?\s*(\d{2,3})\s*°\s*(\d{1,2})\s*'\s*(\d+(?:[.,]\d+)?)\s*""?", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"широта\s*:?\s*(\d{2,3}[.,]\d{4,})\s*°?[,;\s]*(?:WGS\s*)?долгота\s*:?\s*(\d{2,3}[.,]\d{4,})", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"(\d{2,3}[.,]\d{4,})\s*,?\s*°?\s*[NnСс][,;\s]+(\d{2,3}[.,]\d{4,})\s*°?\s*[EeЕеNn]", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"[NnСс]\s*(\d{2,3}[.,]\d{4,})\s*[°'""]?[,;/\s]+[EeЕе]\s*(\d{2,3}[.,]\d{4,})\s*[°'""]?", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"(\d{2,3}[.,]\d{4,})\s*°\s*[СC]\.?\s*[-–—]?\s*(\d{2,3}[.,]\d{4,})\s*°\s*[ВB]\.?", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"(\d{2,3}[.,]\d{4,})\s*[,°]*\s*[СC]\.\s*[-–—,]?\s*(\d{2,3}[.,]\d{4,})\s*°?\s*в\.д\.", RegexOptions.IgnoreCase, 2000)]
    private static partial Regex DecShortNorthRx();

    // «56.123789, E 47.248378» (21-01-04) — метка есть только у долготы.
    [GeneratedRegex(@"(\d{2,3}[.,]\d{4,})\s*°?\s*[,;]\s*(?:E|в\.д\.)\s*:?\s*(\d{2,3}[.,]\d{4,})", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"(\d{1,3})\s*°\s*(\d{1,2})\s*'\s*(\d+(?:\s*[.,]\s*\d+)?)\s*[""°][""'.,;/\s]+[EeЕе]?\s*(\d{2,3})\s*°\s*(\d{1,2})\s*'\s*(\d+(?:\s*[.,]\s*\d+)?)\s*[""°]", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"(\d{2,3}[.,]\d{4,})\s*[°*][,;/\s]+(\d{2,3}[.,]\d{4,})\s*[°*]", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"(\d{2,3}\.\d{4,})\s*\([^)]{5,30}\)\s*[,;/]\s*(\d{2,3}\.\d{4,})\s*\([^)]{5,30}\)", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"(\d{2,3},\d{5,})\s*[,;/\s]\s*(\d{2,3},\d{5,})", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"(\d{2,3}[.,]\d{4,})\s*[NnСс]?\s*\([^)]{4,32}\)[^\d(]{0,14}?(\d{2,3}[.,]\d{4,})\s*[NnEeЕеСсВв]?\s*\([^)]{4,32}\)", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"\d{1,3}\s*°[^()]{2,24}\((\d{2,3}[.,]\d{4,})\)[^()\d]{0,14}?\d{1,3}\s*°[^()]{2,24}\((\d{2,3}[.,]\d{4,})\)", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"(?:с\.ш\.|N)\s*[:=]?\s*(\d{2,3}[.,]\d{3,})\s*[°*]?[^\d]{0,16}?(?:в\.д\.|E)\s*[:=]?\s*(\d{2,3}[.,]\d{3,})", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"широта[^\d]{0,16}(\d{2,3}[.,]\d{4,})[\s\S]{0,60}?долгота[^\d]{0,16}(\d{2,3}[.,]\d{4,})", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"(\d{1,3})\s*°\s*(\d{1,2})\s*'\s*(\d+(?:[.,]\d+)?)\s*""?\s*,?\s*в\.д\.[,;\s]*(\d{2,3})\s*°\s*(\d{1,2})\s*'\s*(\d+(?:[.,]\d+)?)\s*""?\s*,?\s*с\.ш\.", RegexOptions.IgnoreCase, 2000)]
    private static partial Regex DmsSwappedLabelsRx();

    // То же в десятичных градусах: «59.029003 В.Д., 54.675777 С.Ш.» (59-55-20).
    [GeneratedRegex(@"(\d{2,3}[.,]\d{4,})\s*°?\s*в\.д\.[,;\s]*(\d{2,3}[.,]\d{4,})\s*°?\s*с\.ш\.", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"(\d{1,3})\s*°\s*(\d{1,2})\s*'\s*(\d+(?:[.,]\d+)?)\s*""?[\s);]*(?:с\.ш\.|[NС])[.,;/\s]*(\d{2,3})\s*°\s*(\d{1,2})\s*'\s*(\d+(?:[.,]\d+)?)\s*""?", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"(\d{2,3}[.,]\d{4,})\s*,?\s*[°*""']?\s*с\.ш\.[,;/\s]*(\d{2,3}[.,]\d{4,})(?!\d)(?!\s*,?\s*[°*""']?\s*с\.ш)", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"(\d{2,3})\.(\d{2})(\d{2})(?:,(\d))?\s*с\.ш\.[,;\s]*(\d{2,3})\.(\d{2})(\d{2})(?:,(\d))?\s*в\.д\.", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"(\d{2,3})\s+(\d{1,2})(\d{2})[.,](\d+)\s*с\.ш\.[,;\s]*(\d{2,3})\s+(\d{1,2})(\d{2})[.,](\d+)\s*в\.д\.", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"(\d{2,3})\s+(\d{1,2})\s+(\d{1,2}[.,]\d+)\s*с\.ш\.[,;\s]*(\d{2,3})\s+(\d{1,2})\s+(\d{1,2}[.,]\d+)\s*в\.д\.", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"[Nn]\s*(\d{2,3})\s+(\d{1,2})(\d{2})[.,](\d+)\s*[EeЕе]\s*(\d{2,3})\s+(\d{1,2})(\d{2})[.,](\d+)", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"широта\s+(\d{2})[NnСс](\d{2})(\d{2})\s*,\s*долгота\s+(\d{2,3})[EeЕе](\d{2})(\d{2})", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"(\d{2,3})[.,](\d{4,})\s*[,;/]\s*(\d{2,3})[.,](\d{4,})", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"координаты[^\n]{0,25}?(\d{2,3}\.\d{3,})\s*,\s*(\d{2,3}\.\d{3,})", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"(\d{2,3}[.,]\d{4,})\s*°[\s']*[NnСс]\s*/\s*(\d{2,3}[.,]\d{4,})\s*°", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"(\d{2,3})[-.](\d{2})[-.](\d{2}(?:[.,]\d+)?)\s*с\.ш\.[,;\s]*(\d{2,3})[-.](\d{2})[-.](\d{2}(?:[.,]\d+)?)", RegexOptions.IgnoreCase, 2000)]
    private static partial Regex DmsDashDotRx();

    // «55 26' 41,8" с.ш., 82 24' 56,2" в.д.» (54-НС-07) — знак градуса потерян, вместо него пробел.
    [GeneratedRegex(@"(\d{2,3})\s+(\d{1,2})\s*'\s*(\d{1,2}[.,]\d+)\s*""?\s*с\.ш\.[,;\s]*(\d{2,3})\s+(\d{1,2})\s*'\s*(\d{1,2}[.,]\d+)\s*""?\s*в\.д\.", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"(\d{1,3})\s*°\s*(\d{1,2})\s+(\d{1,2}[.,]\d+)\s*""?[,;\s]*с\.ш\.[,;\s]*(\d{2,3})\s*°\s*(\d{1,2})\s+(\d{1,2}[.,]\d+)\s*""?[,;\s]*в\.д\.", RegexOptions.IgnoreCase, 2000)]
    private static partial Regex DmsNoMinuteMarkRx();

    [GeneratedRegex(@"с\.ш\.\s*(\d{1,3})\s*°\s*(\d{1,2})\s+(\d{1,2}[.,]\d+)\s*""?[,;\s]*в\.д\.\s*(\d{2,3})\s*°\s*(\d{1,2})\s+(\d{1,2}[.,]\d+)\s*""?", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"[NnСс](\d{2,3})\.(\d{2})(\d{2})\.(\d{1,2})\s+[EeЕе](\d{2,3})\.(\d{2})(\d{2})\.(\d{1,2})", RegexOptions.None, 2000)]
    private static partial Regex DottedDmsRx();

    // «ш.: 55.772190, д.: 37.678168» — метки-направления ПЕРЕД числами (77-01-09, WGS84)
    [GeneratedRegex(@"ш\.?\s*:\s*(\d{2,3}[\.,]\d{4,})[,;\s]+д\.?\s*:\s*(\d{2,3}[\.,]\d{4,})", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"(\d)\.\s+(\d{4,})", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"(\d)\s*\*(?=\s*(?:\d|[сcСCвВbBnNeEЕе]))", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"(\d)\s*[мМ](?=\s*[СсCcВвEeЕе])", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"(\d{1,3}\s*°\s*\d{1,2}),(?=\d{1,2}[.,]?\d*\s*[""'мМ])", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"(\d{2,3}),\s+(\d{4,})", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"(\d{2,3}\.\d{2,})\s+(\d{1,4})(?=\s*(?:[°""']|[,;]?\s*[свзюСВЗЮ]\s*[.,]\s*[шдШД]))", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"(\d)\s*[оО](?=\s*[сcюСЮвВзЗ]\s*\.?\s*[шШдД])", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"(\d)Г(?=\d{1,2}\s*'\s*\d)", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"(?<=[°\s])(\d)\s*Г\s*(?=\d{1,2}[.,]\d+\s*"")", RegexOptions.None, 2000)]
    private static partial Regex OcrGeMinuteRx();

    // «47°42Г28.6"Е» (05-01-02, 2026) — «Г» проглотила сам знак минуты при ДВУзначных
    // минутах: это 42'28.6".
    [GeneratedRegex(@"(°\s?\d{2})\s*Г\s*(?=\d{1,2}[.,]\d)", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"(\d{1,3}\s*°\s*\d{1,2})\s*\*(?=\s*\d)", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"'\s*Ю(?=\s*[.,]\d)", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"(\d{1,3})(?:[.,]0)?\s*град(?:ус\w*)?\.?\s*(?=\d)", RegexOptions.IgnoreCase, 2000)]
    private static partial Regex WordDegreeRx();

    [GeneratedRegex(@"(\d{1,3})(?:[.,]0)?\s*мин(?:ут\w*)?\.?\s*(?=\d)", RegexOptions.IgnoreCase, 2000)]
    private static partial Regex WordMinuteRx();

    [GeneratedRegex(@"(\d{1,2}[.,]\d+)\s*сек(?:унд\w*)?\.?", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"(\d{2,3})\.(?=\d{1,2}\s*'\s*\d)", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"(\d{2,3}\.\d{3,})\s+(\d{1,3})(?=\s*,\s*\d{2,3}[.,]\d{3,})", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"\b(\d)\.(\d)\s*(?=мин)", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"(\d{1,2})/(?=\d{2}\s*\?)", RegexOptions.None, 2000)]
    private static partial Regex SlashInSecondsRx();

    [GeneratedRegex(@"(\d)\?(?=\d{1,2}[.,]\d)", RegexOptions.None, 2000)]
    private static partial Regex QuestionAsMinuteRx();

    [GeneratedRegex(@"\?(?=\s*[NnEe]\b)", RegexOptions.None, 2000)]
    private static partial Regex QuestionAsQuoteRx();

    // «55гр09'3.5''» (28-22-03) — «гр» вместо знака градуса.
    [GeneratedRegex(@"(\d)\s*гр(?=\s*\d{1,2}\s*['\d])", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"(?<=[NnEeЕе]\s?)(\d{2,3})/(?=\d{4,})", RegexOptions.None, 2000)]
    private static partial Regex SlashDecimalAfterDirRx();

    [GeneratedRegex(@"(\d{2,3})/(?=\d{4,}\s*°)", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"(°\s*\d{1,2})\.(?=\d{1,2}[.,]\d+\s*[""'])", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"(?<=[""'])\s*ст\.(?=\s*,?\s*\d{1,3}\s*°)", RegexOptions.None, 2000)]
    private static partial Regex StAsNorthRx();

    // «39.6"ш. 40°01'…» (23-КК-10) — от метки широты осталась одна «ш.».
    [GeneratedRegex(@"(?<="")\s*[шШ]\.(?=\s*\d{1,3}\s*°)", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"[сcСC][.,]\s*[шщШЩ][.,]?", RegexOptions.None, 2000)]
    private static partial Regex DirNorthRx();

    [GeneratedRegex(@"[вВbB][.,]\s*[дД][.,]?", RegexOptions.None, 2000)]
    private static partial Regex DirEastRx();

    [GeneratedRegex(@"[юЮ][.,]\s*[шщШЩ][.,]?", RegexOptions.None, 2000)]
    private static partial Regex DirSouthRx();

    [GeneratedRegex(@"[зЗ][.,]\s*[дД][.,]?", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"\bс\s*ш\.?(?=\s*[:=]?\s*\d)|(?<=\d[""'°,.\s ])\s*с\s*ш\b", RegexOptions.IgnoreCase, 2000)]
    private static partial Regex DirNorthShortRx();

    [GeneratedRegex(@"\bв\s*д\.?(?=\s*[:=]?\s*\d)|(?<=\d[""'°,.\s ])\s*в\s*д\b", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"(с\.ш\.|в\.д\.|ю\.ш\.|з\.д\.)\.+", RegexOptions.None, 2000)]
