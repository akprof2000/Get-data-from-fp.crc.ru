...
public static partial class AntennaHeightParser
{
    ...
    [GeneratedRegex(@"высот[аы]\s+подвеса[^:\n]{0,60}?[:\s(]\s*(?:высота\s+)?([0-9][0-9.,;/\s-]{0,80}?(?:\sи\s[0-9][0-9.,]{0,6})?)\s*(?:м\b|метр\w*)", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"высота\s+подвеса\s+от\s+уровня\s+(земли|кровли),\s*м\s+((?:\+[0-9]{1,3}(?:[.,][0-9]{1,2})?\s*)+)", RegexOptions.IgnoreCase, 2000)]
    private static partial Regex PodvesPlusListRx();

    // «Высота установки антенны от поверхности земли (м): 29» (71-ТЦ-04)
    [GeneratedRegex(@"высот[аы]\s+установки[^\n(]{0,60}?\(м\)\s*:\s*([0-9]{1,3}(?:[.,][0-9]{1,2})?)", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"высот[аы]\s+установки\s+антенн[^\n(]{0,50}?\(м\)\s*((?:БС|РРС)\s*:[^\n]{1,100})", RegexOptions.IgnoreCase, 2000)]
    private static partial Regex UstSkobkaMetkiRx();

    // «высота установки антенны 45,0 м относительно уровня земли» (55-01-04)
    [GeneratedRegex(@"высот[аы]\s+установки\s+антенн\w*\s+([0-9]{1,3}(?:[.,][0-9]{1,2})?)\s*м\b", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"высот[аы]\s+установки\s+антенн[^:\n]{0,60}:\s*([0-9][0-9.,;/+\s-]{0,80}?)\s*(?:м\b|метр\w*)", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"на\s+отметк[а-яё]*\s+([0-9][0-9.,мМ\s;и]{0,80}?)относительно\s+уровня\s+земли", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"[-–—]\s*([0-9]{1,3}(?:[.,][0-9]{1,2})?)\s*(?:м\b|(?=\s*[;.]?\s*(?:\r?\n|$)))", RegexOptions.Multiline, 2000)]
    private static partial Regex OporSingleRx();

    // «Высоты установки антенн от поверхности земли: 19.5/19.5/19.5;» (86-ХЦ, 2023).
    [GeneratedRegex(@"высот[аы]\s+установки\s+антенн\s+от\s+поверхности\s+земли\s*:\s*([0-9][0-9.,/\s]{0,60}?)(?=\s*[;а-яА-ЯёЁ])", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"([0-9]{1,3}(?:[.,][0-9]+)?)\s+([0-9]{1,3}(?:[.,][0-9]+)?)\s+[0-9]{1,3}\s+-?[0-9]+\s+[0-9]+(?:[.,][0-9]+)?\s*$", RegexOptions.Multiline, 2000)]
    private static partial Regex TailTableRowRx();

    [GeneratedRegex(@"высота\s+подвеса[\s\S]{0,200}?отн\.\s*земли\s+отн\.\s*кровли", RegexOptions.IgnoreCase, 2000)]
    private static partial Regex TailTableHeaderRx();

    // Строка таблицы с высотой «32,5/-» перед режимом работы «Круглосуточно».
    [GeneratedRegex(@"([0-9]{1,3}(?:[.,][0-9]+)?)\s*/\s*(-|[0-9]{1,3}(?:[.,][0-9]+)?)\s+Круглосуточно", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"высот[аы]\s+установки\s+антенн\w*\s+от\s+уровня\s+земли\s*/\s*от\s+уровня\s+кровли\s+([0-9]{1,3}(?:[.,][0-9]{1,2})?)\s*/\s*(-|[0-9]{1,3}(?:[.,][0-9]{1,2})?)\s*м", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"от\s+уровня\s+земли\s*/\s*от\s+опорной\s+поверхности,\s*м", RegexOptions.IgnoreCase, 2000)]
    private static partial Regex TailPairHeaderRx();

    [GeneratedRegex(@"-?[0-9]+/-?[0-9]+\s+([0-9]{1,3}(?:[.,][0-9]{1,2})?)/(-|[0-9]{1,3}(?:[.,][0-9]{1,2})?)\s*$", RegexOptions.Multiline, 2000)]
    ...
    [GeneratedRegex(@"от\s+поверхности\s+земли\s*\(кровли\),\s*м\s*;", RegexOptions.IgnoreCase, 2000)]
    private static partial Regex ZemKrovTblHeaderRx();

    [GeneratedRegex(@"\s([0-9]{1,3}(?:[.,][0-9]{1,2})?)/([0-9]{1,3}(?:[.,][0-9]{1,2})?|-)\s+[0-9]{1,3}\s+-?[0-9]+/-?[0-9]+\s", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"(?:мачт|опор|башн|столб|трубостойк)[а-яё]*[^\n.;]{0,60}?высотой\s+([0-9]{1,3}(?:[.,][0-9]{1,2})?)\s*м\b", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"высот[аы][^\n:]{0,80}?,\s*м\s*[-–—]\s*([0-9]+(?:[.,][0-9]+)?)(?:\s*\(([^)\n]{1,12})\))?", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"высота\s+подвеса[\s\S]{0,300}?координаты\s+установки", RegexOptions.IgnoreCase, 2000)]
    ...
    [GeneratedRegex(@"\s([0-9]{1,3}(?:[.,][0-9]+)?)\s+(-|[0-9]{1,3}(?:[.,][0-9]+)?)\s+'?[0-9]{1,3}(?:\s*-\s*[0-9]{2,3})?\s+-?[0-9]+(?:[.,][0-9]+)?\s+-?[0-9]+(?:[.,][0-9]+)?;-?[0-9]+", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"\s([0-9]{1,3}(?:[.,][0-9]{1,2})?)\s+(-|[0-9]{1,3}(?:[.,][0-9]{1,2})?)\s+[0-9]{1,3}\s+-?[0-9]+\s+-?[0-9]+(?:[.,][0-9]+)?\s+-?[0-9]+(?:[.,][0-9]+)?\s+RF\b", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"[:\-–—,]\s*([0-9]{1,3}(?:[.,][0-9]{1,2})?)\s*/\s*(-|[0-9]{1,3}(?:[.,][0-9]{1,2})?)(?=[,;\s]|м)", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"высота\s+подвеса\s+относительно\s+земли,\s*м\s+высота\s+подвеса\s+относительно\s+кровли", RegexOptions.IgnoreCase, 2000)]
    private static partial Regex DvaColHeaderRx();

    [GeneratedRegex(@"^\d{1,2}\s+\S[^\n]*?\s([0-9]{1,3}(?:[.,][0-9]{1,2})?)\s+(-|[0-9]{1,3}(?:[.,][0-9]{1,2})?)\s+[0-9]{1,3}(?:[.,][0-9]{1,2})?\s", RegexOptions.Multiline, 2000)]
    ...
    [GeneratedRegex(@"высота\s+подвеса,\s*м\s*\(отн\.\s*земли;\s*отн\.\s*кровли\)", RegexOptions.IgnoreCase, 2000)]
    private static partial Regex SemiTableHeaderRx();

    [GeneratedRegex(@";\s*([0-9]{1,3}(?:[.,][0-9]{1,2})?);\s*(-|[0-9]{1,3}(?:[.,][0-9]{1,2})?);\s*[0-9]{1,3};\s*-?[0-9]+(?:/-?[0-9]+)?;\s*-?[0-9]+;-?[0-9]+;", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"[А-Я]\s*[-–—]\s*Высота\s+установки\s+антенн\w*\s+от\s+поверхности\s+земли", RegexOptions.IgnoreCase, 2000)]
    private static partial Regex LetterTableHeaderRx();

    [GeneratedRegex(@"\s([0-9]{1,3}(?:[.,][0-9]{1,2})?)\s+0\s*-\s*360\s", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"\s([0-9]{1,3}(?:[.,][0-9]{1,2})?)\s+[0-9]{1,3}\s+-?[0-9]+/-?[0-9]+\b", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"\s([0-9]{1,3}(?:[.,][0-9]{1,2})?)\s+[0-9]{1,3}\s+(?:-[0-9]{1,2}|0)\s*$", RegexOptions.Multiline, 2000)]
    private static partial Regex LetterTableRow3Rx();

    // Заголовок перечня «по каждой антенне»: высота — второе поле записи после «шт.,».
    [GeneratedRegex(@"высот[аы]\s+установки\s+антенн\w*\s+от\s+поверхности\s+земли\s*,", RegexOptions.IgnoreCase, 2000)]
    private static partial Regex ShtHeaderRx();

    // Запись перечня: «<тип> - 3 шт., 43 м, …».
    [GeneratedRegex(@"шт\.?\s*,\s*([0-9]+(?:[.,][0-9]+)?)\s*м\b", RegexOptions.IgnoreCase, 2000)]
    private static partial Regex ShtItemRx();

    // Заголовок табличной колонки высот с плюсовой пометкой значений.
    [GeneratedRegex(@"высота\s*\(м\)\s*(?:подвеса|фазового\s+центра)|высота\s+подвеса\s+от\s+уровня\s+земли,\s*м\s*;", RegexOptions.IgnoreCase, 2000)]
    private static partial Regex PlusTableHeaderRx();

    // «+61,0» — высота из строки такой таблицы (плюс её и метит).
    [GeneratedRegex(@"\+([0-9]{1,3}(?:[.,][0-9]{1,2})?)\b", RegexOptions.None, 2000)]
    ...
    [GeneratedRegex(@"(\d{1,3}(?:[.,]\d{1,2})?)", RegexOptions.None, 2000)]
    private static partial Regex LooseNumberRx();

    [GeneratedRegex(@"^\s*(\d+(?:[.,]\d+)?)\s*-\s*(\d+(?:[.,]\d+)?)\s*$", RegexOptions.None, 2000)]
