...
public sealed partial class AddressMatcher
{
    ...
    private (Kind, string, string?) ClassifySegment(string seg)
    {
        ...
        lc = lc.Replace("городской округ", "го")
               ...
               .Replace("м.о.", " мо ")
               .Replace("г.о.", " го ")
               // …и с пробелом между точками: «г. о. Богородский» (50-99-02).
               .Replace("г. о.", " го ")
               .Replace("м. о.", " мо ")
               .Replace("м.р-н", " рн ")
        ...
        foreach (var w in words)
        {
            ...
            if (wClean.Length > 0 && TypeMarkers.TryGetValue(wClean, out var k))
            {
                ...
                // То же для НП, когда имя уже началось: «д. Новая Деревня» (35-ВЦ-02).
                else if (kind == Kind.Settlement && k == Kind.Settlement && nameWords.Count > 0) { nameWords.Add(w); }
                continue;
            }
    ...
    private static readonly Dictionary<string, string[]> SettlementTypeSynonyms = new()
    {
        ...
        ["с"] = ["с"], ["село"] = ["с"],
        ["п"] = ["п"], ["пос"] = ["п"], ["поселок"] = ["п"],
        // «пгт» и «рп» взаимозаменяемы: «пгт. Кадуй» в документе — «рп. Кадуй» в ГАР (35-ВЦ-02).
        ["рп"] = ["рп", "пгт"], ["пгт"] = ["пгт", "рп"],
        ["д"] = ["д"], ["дер"] = ["д"], ["деревня"] = ["д"],
        ["х"] = ["х"], ["хутор"] = ["х"], ["аул"] = ["аул"],
