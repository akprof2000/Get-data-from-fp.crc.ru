...
CREATE TABLE IF NOT EXISTS {database}.{table} (
    ...
    antenna_heights Array(Float64),
    antenna_height_bases Array(String),
    antenna_models Array(String),
    antenna_numbers Array(Int32),
    ...
    coords_conflict_place_km Nullable(Float64),
    coords_conflict_street Nullable(String),
...
def parse_json_file(file_path: Path, logger: logging.Logger) -> Optional[Dict[str, Any]]:
    ...
    record = {
        ...
        "coords_conflict_place_km": (data.get("addressByCoords") or {}).get("placeDistanceKm"),
        "coords_conflict_street": (data.get("addressByCoords") or {}).get("street"),
        "coords_conflict_km": (data.get("addressByCoords") or {}).get("parsedAddressDistanceKm"),
        "coords_conflict_source": (data.get("addressByCoords") or {}).get("source"),
        # Незаполненные обязательные поля разбора (единая выходная папка, без OutputErrors).
...
class ClickHouseWriter:
    ...
    (`index`, document_number, document_date, base_station_number, base_station_address,
     lat, lon, operator, antenna_heights, antenna_height_bases, antenna_models, antenna_numbers,
     ...
     addr_region_code, addr_lat, addr_geo_level, addr_lon,
     coords_conflict_place, coords_conflict_place_km, coords_conflict_street,
     coords_conflict_km, coords_conflict_source, missing_fields,
    ...
    def insert_batch(self, rows: List[Dict[str, Any]]) -> int:
        ...
        data = [
            (
                ...
                r["coords_conflict_place_km"],
                r["coords_conflict_street"],
                r["coords_conflict_km"],
                r["coords_conflict_source"],
                r["missing_fields"],
                r["processing_date"],
                r["source_file_name"],
                r["relative_path"],
